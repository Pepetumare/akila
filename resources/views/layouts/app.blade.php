...
<html lang="es"
      ...
      x-cloak
      class="min-h-screen bg-gray-50 scroll-smooth">
    ...
    <header class="bg-red-600 text-white shadow-md">
        <div class="container mx-auto flex items-center justify-between p-4">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2 group">
                <img src="{{ asset('img/logo/logo-light-transparent.png') }}"
                     alt="Logo Sushi Akila"
        ...
        <nav x-show="mobileMenuOpen"
            ...
            {{-- <a href="{{ route('menu', ['section' => 'promociones']) }}"
               ...
               @guest
               <a href="{{ route('login') }}"    class="block py-1 text-white no-underline hover:text-yellow-300">Ingresar</a>
               <a href="{{ route('register') }}" class="block py-1 text-white no-underline hover:text-yellow-300">Registrarse</a>
               @else
               <a href="{{ route('profile') }}"  class="block py-1 text-white no-underline hover:text-yellow-300">Mi Cuenta</a>
               <form method="POST" action="{{ route('logout') }}">
                   @csrf
                   <button type="submit" class="w-full text-left py-1 text-white no-underline hover:text-yellow-300">Cerrar Sesión</button>
                </form>
                @endguest
                <a href="{{ route('cart.index') }}" class="block py-1 no-underline text-white hover:text-yellow-300">Carrito</a>
        </nav>
    </header>
