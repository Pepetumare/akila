                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($productos as $prod)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500">${{ number_format($prod->precio, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $prod->personalizable ? 'Sí' : 'No' }}</td>
                            <td class="px-6 py-4 text-right text-sm font-medium space-x-2">
                                <button type="button"
                                    @click="openEdit({{ $prod->id }},'{{ addslashes($prod->nombre) }}',{{ $prod->categoria_id ?? 'null' }},{{ $prod->precio }},{{ $prod->personalizable ? 'true' : 'false' }})"
        ...
        <template x-if="activeModal === 'delete'">
            <div class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                <div class="bg-white rounded-lg shadow-lg w-full max-w-sm mx-4 my-8 p-6">
                    ...
                    <p class="mb-4">
                        ¿Eliminar el producto <strong x-text="deleteName"></strong>?
                    </p>
                    <div class="flex justify-end space-x-2">
                        <button type="button" @click="closeModal()"
                            class="px-4 py-2 border rounded hover:bg-gray-100">Cancelar</button>
