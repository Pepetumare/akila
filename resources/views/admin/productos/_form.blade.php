...
<div class="mb-4">
    ...
    <textarea name="descripcion" id="descripcion" class="border p-2 w-full" rows="3">{{ old('descripcion', $producto->descripcion ?? '') }}</textarea>
    @error('descripcion')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
    @enderror
</div>
...
<div class="mb-4">
    {{-- Hidden para asegurar siempre envío --}}
    <input type="hidden" name="personalizable" value="0">

    <label class="inline-flex items-center">
        <input type="checkbox" name="personalizable" value="1"
            {{ old('personalizable', $producto->personalizable ?? false) ? 'checked' : '' }} class="form-checkbox">
...
<div class="mb-4">
    ...
    <input type="file" name="imagen" id="imagen" class="border p-2 w-full">
    ...
    <div class="mb-4">
        ...
        <input type="number" name="unidades" id="unidades" value="{{ old('unidades', $producto->unidades ?? 1) }}"
        ...
        <div class="mb-4">
            ...
            <select name="swappables[]" id="swappables" multiple class="border p-2 w-full h-32">
                @foreach (\App\Models\Ingrediente::where('tipo', 'extra')->get() as $ing)
                    <option value="{{ $ing->id }}"
                        {{ in_array($ing->id, old('swappables', isset($producto) ? $producto->swappables->pluck('id')->toArray() : []))
                            ? 'selected'
                            : '' }}>
                        {{ $ing->nombre }}
                    </option>
